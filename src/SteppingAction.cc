...
=======
  ...
  /*if(name == "gamma" && volume_name=="Polyethile_Shielding" && volume_name_post=="World"){  // after shielding
    //// primo step
     if(pdgcode==11)fRunAction->EventInfoRecoilFirst(creatorID,whichInter,kinene,StepNo,trackID,parentID,&prePointPos,&momvect,tracklength,pdgcode,whichVol,edep);
     }*/
  ...
  if(whichVol==1 || whichVol==2){
    if (kinene!=0 && (pdgcode==11 || pdgcode==-11 || pdgcode==2212 || pdgcode>=1000000000 || pdgcode==0 /*|| pdgcode==22*/)){
      ...
      fRunAction->EventInfoRecoilLast(creatorID,whichInter,kinene,StepNo,trackID,parentID,&prePointPos,&postPointPos,&momvect,tracklength,pdgcode,whichVol,edep,copyNo);
    }
  }
